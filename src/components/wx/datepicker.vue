  ...
  methods: {
    ...
    isstartdate(date) {
      if (this.type == "starttime") {
        ...
        if (
          this.startob.year == this.curYear &&
          this.startob.month == this.curMonth &&
          this.startob.date == date
        ) {
          return true;
        } else {
          return false;
    ...
    isbeforeTodayDays(date) {
      ...
      if (this.curYear > endyear) {
        return true;
      }
      if (this.curYear == endyear && this.curMonth > endmonth) {
        return true;
      }
      if (date > enddate && this.curMonth == endmonth) {
        return true;
      }
    ...
    clickspan(n, n2) {
      ...
      if (this.istoken(this.dateform[n][n2])) {
        //判断是否已租，已租不能点
        return;
      } else {
    ...
    su() {
      if (this.type == "starttime") {
        ...
        for (let i = 0; i <= this.tokendays.length; i++) {
          if (
            new Date(this.tokendays[i]) < endstring &&
            new Date(this.tokendays[i]) > startstring
          ) {
            alert("起止日期间不允许间隔");
            return false;
          }
